...
for unknown_image in os.listdir(unknown_images_folder_path):                        # Ref: https://stackoverflow.com/questions/1120707/using-python-to-execute-a-command-on-every-file-in-a-folder
    ...
    # To make face recongnition (face_recognition module):
    ...
    unknown_encodings = face_recognition.face_encodings(image_fac_rec_unknown)      # Getting list with encodings of faces found.
    ...
    for unknown_enc in unknown_encodings:
        ...
        index = min(range(len(distances)), key=distances.__getitem__)                   # Getting index for most similar person (lower distance). Ref: https://stackoverflow.com/questions/2474015/getting-the-index-of-the-returned-max-or-min-item-using-max-min-on-a-list
